...
module "ecs" {
  ...
  whitelist_ips    = var.env == "uat" || var.env == "oat" ? ["0.0.0.0/0"] : jsondecode(data.aws_secretsmanager_secret_version.terraform_secret_version.secret_string)["whitelist-ips"]
